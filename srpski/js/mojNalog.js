...
function ucitaj(){ 
    ...
    if(localStorage.getItem("ulogovaniKorisnik")!=null) { 
        ...
        document.getElementById("dodajOvde").innerHTML="<div class='col-sm-12 sredina text-center'><h2><hr>Moji oglasi<hr></h2></div>"
           ...
           ' <div class="card" style="width:100%; margin-top: 10px; id="content">'+
               ...
               ' <h4 class="cardtitle">'+element.naziv_zivotinje+'<div style="text-align:right"><button class="btn dugmeBrisi" style="margin-top:-50px; width:40px;font-size:8pt" onclick="brisi('+element.id+')"><i class="fa fa-trash-o" style="font-size:20px"></i></button></div><hr style="margin-top:-15px;"></h4>'+''+
                    '<p class="cardtext">'+element.opis+
                           '<p class="cardtext"> Datum postavke oglasa:<b>'+element.datumVreme+      
               ' </p></p></div> '+
               ' <button class="btn dugmence" id="pregledajKomentar'+element.id+'" onclick="pregledKomentara('+element.id+')"'+element.id+'">Pregledaj komentare</button></p>'+
                '</div> </div>';}
              
        });
    };

        document.getElementById("dodajOvde").innerHTML+="<div class='col-sm-12 sredina text-center'><br><br><br><h2><hr>Moji komentari<hr></h2></div>"
        niz=Array();
        oglasi.forEach(element=>{
        ...
        if(niz.length==0){ 
            document.getElementById("dodajOvde").innerHTML+='<div class="col-sm-12 text-center sredina" style="font-size:16pt;">Niste ostavili nijedan komentar!</div>'; 
